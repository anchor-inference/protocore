...
- `runtime/context/budgets.py` — `derive_budgets` превращает один снимок RC во
  ...
  примет, — окно минус резерв вывода (`llm_output_max_tokens_ratio`), минус
  `request_context_safety_tokens`, минус доля окна
  `compaction_trigger_turn_headroom_ratio` на тот ход, который вот-вот
  добавится. Сервер, резервирующий бюджет вывода внутри окна, отвергает любой
  промпт больше `окно − максимум вывода`, поэтому порог, выведенный из одной
  лишь доли, может оказаться выше обрыва и не сработать никогда: на окне в
  65 536 токенов при штатном резерве 0.25 доля 0.8 лежит на 3 276 токенов
  дальше точки, после которой запрос перестают принимать. Потребители читают
  эффективное значение; аварийный обрыв держится строго выше него.
- `runtime/context/compaction.py` — три прохода по транскрипту, по порядку,
  ...
  **Tier 1** заменяет вышедший за бюджет результат инструмента плейсхолдером,
  ...
  и не фиксируется), вызовы уходят по
  ...
  Вызов, который вообще не может завершиться — провайдер бросил исключение,
  запрос не помещается, в ответе нет читаемого саммари, — записывается на СВОЙ
  юнит в `CompactionState.failed_anchor_keys`, и после
  `compaction_summary_failed_unit_max_attempts` этот юнит больше не
  отправляется; у яруса свёртки остаётся своя попытка. Остальные юниты пакета
  фиксируются в любом случае, поэтому один непосильный для суммаризатора юнит
  больше не обнуляет весь проход. Перепись едет в снимке прогона, так что
  прогон, перезапущенный на другом поде, не начинает её с нуля. Саммари, просто
  оказавшееся не меньше оригинала, не засчитывается: это факт о размере юнита,
  а не о том, может ли вызов завершиться.
...
- `runtime/stale_result_trim.py` — сокращение промпта, которое не стоит ни
  ...
  `Cite exactly:` / `Cite:` / `cite_as:` / `catalog_url:`), переносится
