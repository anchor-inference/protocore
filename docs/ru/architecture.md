...
- `runtime/context/compaction.py` — три прохода по транскрипту, по порядку,
  ...
  **Tier 1** заменяет вышедший за бюджет результат инструмента плейсхолдером,
  ...
  `compaction_failed_max_retries`; за его пределом `CompactionExhaustedError`
  завершает уплотнение. Списывается только проход, который пытался и не смог, —
  ярус выбросил исключение или вызов суммаризатора был сделан и ничего не дал,
  — и списывается один раз, сколько бы ярусов ни упало. Проход, не нашедший
  ничего, что его профилю разрешено трогать, не тратит ничего: обычный случай —
  проактивный аварийный проход по истории из засеянных ходов. Обычные и
  проактивные проходы делят `CompactionState.retry_count`; реактивный проход
  ...
  проактивных проходов о нём ничего не доказывают. Любой прогресс обнуляет
  бюджет того прохода, что его дал. Сбой транспорта поэтому стоит проходу один
  повтор, а юниту ничего; сбой, принадлежащий юниту, засчитывается ещё и юниту.
  Принудительные проходы перепись игнорируют в любом случае. Оба счётчика едут
  в снимке прогона.
