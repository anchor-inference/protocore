...
- `runtime/request_budget.py` — подгоняет каждый собранный запрос под жёсткое
  ...
  Точный подсчёт выставляет `token_estimate_calibration` сразу; отчёт `usage`
  после вызова сдвигает его наполовину; отказ по длине поднимает его до нижней
  границы, которую отказ доказывает (промпт был не меньше окна за вычетом
  отправленного лимита вывода). Подсчёты хранятся по содержимому запроса
  (`exact_token_count_cache_max_entries`); неудавшийся подсчёт пишется в лог, и
  используется оценка; провайдер без этой возможности отправляет ровно те же
  запросы, что и раньше. Каждый подсчёт пишет в лог оценку рядом с измерением —
