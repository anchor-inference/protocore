...
- `runtime/context/compaction.py` — три прохода по транскрипту, по порядку,
  ...
  **Tier 1** заменяет вышедший за бюджет результат инструмента плейсхолдером,
  ...
  историей, которую она видела, и не спрашивает снова, пока история не
  изменится. За пределом бюджета проактивный проход не завершает прогон:
  провайдер ещё ничего не отклонил, поэтому проактивное уплотнение
  приостанавливается (одно изменение состояния
  `compaction_exhausted_proactive_suspended`), и запрос уходит. Следующий отказ
  провайдера снимает приостановку и запускает реактивный проход. Реактивный
  проход за пределом своего бюджета передаёт ход лестнице лимита вывода, пока
  есть меньший лимит, и проваливает прогон, только когда его нет.
