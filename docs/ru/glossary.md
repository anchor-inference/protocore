...
: **Двухъярусный** каскад, удерживающий промпт под окном контекста провайдера на
  ...
  `CompactionExhaustedError` приостанавливает проактивное уплотнение до отказа
  провайдера; после отказа он передаёт ход лестнице лимита вывода, и лишь когда
  меньшего лимита не осталось, цикл переходит в `FAILED`. Операторский `/compact`
